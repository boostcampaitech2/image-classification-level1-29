...
import torch
from torch.optim.lr_scheduler import CosineAnnealingWarmRestarts, StepLR, CosineAnnealingLR, ReduceLROnPlateau
...
def train(data_dir, model_dir, args):
    ...
    for train_split, model in zip(TrainSplitNum[args.train_split], args.models.split(',')):
        ...
        # # -- mode
        # if train_split == 'all':
        #     model_module = load_model(model_dir, num_classes, device) # default: BaseModel
        # else:
        #     model_module = getattr(import_module("model"), model)
        # model = model_module(
        #     num_classes=num_classes
        ...
        if args.scheduler == "StepLR":
            ...
            scheduler = CosineAnnealingWarmRestarts(optimizer,T_0=100,T_mult=1, eta_min=0.00001)
        else:
            raise ValueError
        ...
        for epoch in range(args.epochs):
            ...
            #     best_val_loss = min(best_val_loss, val_loss)
                if train_acc*0.95 > best_train_acc:
                       
                    valid_early_stop=0
                    print(f"New best model for val accuracy : {train_acc:4.2%}! saving the best model..\n")
                    if train_split == 'all':
