...
class TrainandEval():
    ...
    def __init__(self,num_epoch,train_loader,valid_loader,model,optimizer,lr_scheduler):
        ...
        best_model = self.learn()
        self.model = best_model



    def learn(self):
        config={'epochs':hp.num_epoch,'batch_size':hp.batch_size,'learning_rate':hp.initial_lr}
        ...
        for e in range(self.num_epoch):
            ...
            for i, (images,targets) in enumerate(tqdm(self.valid_loader)):
                ...
                wandb.log({'valid loss' : np.sum(valid_loss_list)/(i+1), 'valid_accuracy' : (np.sum(float(valid_acc_list))/(i+1)),
            'valid_f1 score':f1_score(targets.cpu(),preds.cpu(),average='macro')})
